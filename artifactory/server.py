...
# --- TLS trust ---
# Behind a corporate TLS-inspecting proxy (Zscaler, Netskope, ...) the upstream
# certificate is re-signed by a private root. Set SSL_CERT_FILE to a bundle
# containing the public roots plus that corporate root.
#
# Some corporate roots predate current X.509 strictness — a common one is a
# basicConstraints extension that is not marked critical. Python 3.13 enables
# ssl.VERIFY_X509_STRICT in create_default_context(), which rejects those with
# "Basic Constraints of CA cert not marked critical". Setting this flag relaxes
# only those RFC 5280 conformance checks. Certificate chain validation, expiry
# and hostname verification all remain enabled.
ARTIFACTORY_TLS_RELAX_STRICT = os.getenv(
    "ARTIFACTORY_TLS_RELAX_STRICT", ""
).lower() in ("1", "true", "yes")


def _build_ssl_context() -> ssl.SSLContext:
    """TLS verification context for every outbound JFrog call."""
    ctx = ssl.create_default_context(cafile=os.getenv("SSL_CERT_FILE") or None)
    if ARTIFACTORY_TLS_RELAX_STRICT:
...
if os.getenv("SSL_CERT_FILE"):
    print(f"TLS: custom CA bundle {os.getenv('SSL_CERT_FILE')}"
          f"{' (X509 strict checks relaxed)' if ARTIFACTORY_TLS_RELAX_STRICT else ''}")
if SECURE_MODE:
