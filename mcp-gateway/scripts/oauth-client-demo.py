...
def main():
    ...
    ap.add_argument("--client-type", default=None,
                    help="value for the x-mcp-client-type attribution header "
                         "(e.g. 'agent' or 'coding-assistant'); drives the "
                         "agent-vs-human breakdown in the monitoring dashboard")
    ...
    if args.client_type:
        # Attribution header: the gateway records this so the dashboard can
        # distinguish agent traffic from interactive coding-assistant traffic.
        auth["x-mcp-client-type"] = args.client_type
        print(f"    attribution: x-mcp-client-type={args.client_type}")
