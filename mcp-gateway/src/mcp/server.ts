...
import { recordRejection, RejectionReason } from '~/metrics'
...
export class McpServer {
  ...
  async handleRequest(
    ...
    if (this.requiresAuth) {
      ...
      if (this.#config.forward_id_token) {
        if (!accessToken.id_token) {
          recordRejection({
            reason: RejectionReason.MissingIdToken,
            namespace: this.#name,
    ...
    if (rateLimiter && accessToken) {
      ...
      if (await rateLimiter.checkAll(key)) {
        recordRejection({
          reason: RejectionReason.RateLimitedServer,
          namespace: this.#name,
          userId: accessToken.user_id,
          provider: accessToken.provider,
          clientId: accessToken.client_id,
      ...
      if (toolName && (await rateLimiter.checkTool(toolName, key))) {
        recordRejection({
          reason: RejectionReason.RateLimitedTool,
          namespace: this.#name,
          userId: accessToken.user_id,
          provider: accessToken.provider,
          clientId: accessToken.client_id,
