...
def llm_eval():
    ...
    if tracking is None:
        st.error(
            "Tracking backend unavailable: "
            + str(st.session_state.get("tracking_error", "unknown error"))
        )
        return

    s = state.effective_settings()
    st.caption(f"Tracking backend: **{s.tracking_backend}**" + (f" · `{s.bq_project_id}.{s.bq_dataset_id}`" if s.tracking_backend == "bigquery" else ""))

    try:
        summary = tracking.llm_summary()
        fb = tracking.feedback_summary()
        calls = tracking.list_llm_calls(500)
    except Exception as e:  # noqa: BLE001
