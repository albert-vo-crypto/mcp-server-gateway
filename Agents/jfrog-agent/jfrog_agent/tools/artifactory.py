...
def search_artifacts(
    ...
    """
    # A generic "list packages" request may arrive with no structured intent
    # (the planner omitted or emptied it). Default to a match-all search.
    if intent is None:
        intent = SearchIntent()
    ...
    if repo:
        args["repo"] = repo
    # The upstream tool rejects an empty query; default to match-all so a generic
    # "list artifacts/packages" request returns results instead of a 400.
    if not args:
        args["name"] = "*"
