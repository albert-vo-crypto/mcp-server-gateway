...
class GatewayMCPClient:
    def __init__(self, settings: Settings = default_settings, *, verbose: bool = True):
        ...
        # The gateway routes on the Host header — it only answers to its configured
        # base_url origin. When this client runs in a container it *connects* to
        # gateway_url (e.g. http://host.docker.internal:8090) but the gateway still
        # expects Host = its public origin (localhost:8090), so pin it here. In a
        # plain local run public == gateway_url, so this is a harmless no-op.
        default_headers: dict[str, str] = {}
        host = urllib.parse.urlsplit(self.s.gateway_public_url).netloc
        if host:
            default_headers["Host"] = host
        self._http = httpx.Client(
            timeout=60.0, follow_redirects=False, headers=default_headers
        )
    ...
    def has_valid_token(self) -> bool:
        """True if a usable access token is available without a browser.

        Gateway access tokens are short-lived (typically one hour), so checking
        expiry alone would force a fresh browser login every hour even when the
        cached refresh_token can renew silently. Try that refresh first; only
        report False when there is genuinely nothing left to use.
        """
        ...
        if not cache.get("refresh_token"):
            return False
        try:
            refreshed = self._refresh(self._discover(), cache)
        except Exception:  # noqa: BLE001 - offline/unreachable gateway
            return False
    ...
    def _reconnect(self, *, force_auth: bool = False) -> None:
        """Re-open the MCP session (and refresh the token if needed)."""
        if force_auth:
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            self._reconnect(force_auth=True)
    ...
    def _is_recoverable_session_error(self, exc: Exception) -> bool:
        ...
        if isinstance(exc, MCPError):
            msg = str(exc).lower()
            return "session not found" in msg or "401" in msg or "unauthorized" in msg
        return False
    ...
    def call_tool(self, name: str, arguments: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            if not self._is_recoverable_session_error(exc):
                raise
            # Session or token went stale mid-run (common after gateway/Redis restart).
            self._log("Tool call auth/session error; reconnecting and retrying once…")
            self._reconnect()
            result = self._rpc("tools/call", {"name": name, "arguments": arguments or {}})
