...
class Settings:
    ...
    # backend: "sqlite" (default, local llm_calls table), "bigquery" (production
    # eval pipeline), or "none" (discard).
    tracking_backend: str = os.getenv("JFROG_AGENT_TRACKING_BACKEND", "sqlite").lower()
    tracking_db: Path = field(
        default_factory=lambda: _expand(
            os.getenv("JFROG_AGENT_TRACKING_DB", os.getenv("JFROG_AGENT_MEMORY_DB", "~/.jfrog-agent/memory.db"))
        )
    )
    bq_project_id: str = os.getenv(
        "JFROG_AGENT_BQ_PROJECT_ID", os.getenv("AGENT_EVAL_BQ_PROJECT_ID", "bqgcs-177117")
    )
    bq_dataset_id: str = os.getenv(
        "JFROG_AGENT_BQ_DATASET_ID", os.getenv("AGENT_EVAL_BQ_DATASET_ID", "agent_tracking_eval")
    )

    # ── Context optimizer (layered token budget before each LLM call) ────────
    # Uses CTX_OPT_* env vars (same names as the BigQuery template agent).
    ctx_opt_enabled: bool = os.getenv("CTX_OPT_ENABLED", "").strip().lower() in {
        "1", "true", "yes", "on",
    }
    ctx_opt_preset: str = os.getenv("CTX_OPT_PRESET", "balanced")
    ctx_opt_max_tokens: int = int(os.getenv("CTX_OPT_MAX_TOKENS", "8000") or "8000")
    ctx_opt_model: str = os.getenv("CTX_OPT_MODEL", "gpt-4o-mini")
