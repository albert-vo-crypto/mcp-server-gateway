...
from jfrog_agent.mcp_client import AuthError
...
def home():
    ...
    section(
        ...
        "Xray posture, investigations and recommendations are illustrative — they require "
        "the JFrog Xray/Build APIs, which are wired in Phase 2.",
    )
    ...
    preview_banner("Xray posture and recommendations are illustrative. Repositories, storage, "
                   "artifact search and audit use live data from your JFrog trial.")


# ── Live overview (real data via MCP tools) ───────────────────────────────────
def _live_overview():
    ov = state.fetch_overview()

    head, refresh = st.columns([6, 1])
    with head:
        section(
            "Live from your JFrog trial",
            "Repositories and storage read from Artifactory through the MCP gateway, "
            "scoped to your OAuth identity.",
        )
    with refresh:
        if st.button("↻ Refresh", key="ov_refresh", width="stretch"):
            state.fetch_overview(force=True)
            st.rerun()

    if not ov.get("connected"):
        st.info(
            "Not connected yet — authorize once on the **Ask JFrog Agent** page and this "
            "panel fills with your real repository and storage data."
        )
        return

    if ov.get("repos") is None and ov.get("error"):
        st.error(f"Could not read live data from Artifactory: {ov['error']}")
        return

    repos = ov.get("repos") or []
    storage = ov.get("storage") if isinstance(ov.get("storage"), dict) else {}
    binaries = storage.get("binariesSummary", {}) if storage else {}
    filestore = storage.get("fileStoreSummary", {}) if storage else {}

    metric_row(
        [
            {"label": "Repositories", "value": f"{len(repos):,}"},
            {"label": "Artifacts", "value": str(binaries.get("artifactsCount", "—"))},
            {"label": "Binaries size", "value": str(binaries.get("binariesSize", "—"))},
            {"label": "Used space", "value": str(filestore.get("usedSpace", "—"))},
        ]
    )

    st.write("")
    section("Repositories")
    summary_list = storage.get("repositoriesSummaryList") if storage else None
    if summary_list:
        rows = [
            {
                "Repository": r.get("repoKey"),
                "Type": r.get("repoType"),
                "Package": r.get("packageType"),
                "Files": r.get("filesCount"),
                "Used": r.get("usedSpace"),
            }
            for r in summary_list
            if r.get("repoKey") not in (None, "TOTAL")
        ]
        st.dataframe(pd.DataFrame(rows), width="stretch", hide_index=True)
    elif repos:
        rows = [
            {
                "Repository": r.get("key"),
                "Type": r.get("type"),
                "Package": r.get("packageType"),
                "URL": r.get("url"),
            }
        ...
        st.caption("No repositories visible to your account.")

    if ov.get("error"):
        st.caption(f"⚠️ Storage detail unavailable ({ov['error']}); repository list is live.")
    st.caption("✅ Live data from your JFrog trial via the MCP gateway (per-user OAuth).")
            ...
            with st.chat_message("assistant"):
                with st.spinner("Working through the graph…"):
                    try:
                        ...
                        st.info("Your gateway session expired (common after a Docker restart). Re-authorize below.")
                        state.mark_disconnected()
                        if st.button("🔐 Re-authorize with gateway", type="primary", key="reauth_after_run"):
                            state.do_login()
                        return
...
def _record_feedback(run_id: str, rating: int):
    tracking = state.get_tracking_backend()
    if tracking:
        try:
            tracking.record_feedback(run_id, state.current_thread_id(), rating)
        except Exception:
            pass
...
def _context_panel():
    ...
    section("Token usage (last run)")
    tokens = st.session_state.get("last_run_tokens") or {}
    ctx = st.session_state.get("last_ctx_opt") or {}
    s = state.effective_settings()
    with st.container(border=True):
        if tokens.get("calls"):
            kv("LLM calls", str(tokens.get("calls")))
            kv("Input tokens", f"{int(tokens.get('input_tokens', 0)):,}")
            kv("Output tokens", f"{int(tokens.get('output_tokens', 0)):,}")
            kv("Total tokens", f"{int(tokens.get('total_tokens', 0)):,}")
            kv("Est. cost", f"${float(tokens.get('cost', 0)):.5f}")
            by_p = tokens.get("by_purpose") or {}
            if by_p:
                kv("By step", ", ".join(f"{k}: {v:,}" for k, v in by_p.items()))
        else:
            st.caption("Run a query to see token usage.")
        kv("Context optimizer", "ON" if s.ctx_opt_enabled else "OFF")
        if ctx.get("calls"):
            kv("Ctx-opt calls", str(ctx.get("calls")))
            kv("Ctx before → after", f"{ctx.get('tokens_before', 0):,} → {ctx.get('tokens_after', 0):,}")
            kv("Ctx tokens saved", f"{ctx.get('saved', 0):,}")
        st.caption("Governance → LLM & Evaluation shows history across all runs.")
