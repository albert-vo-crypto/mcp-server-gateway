...
def build_items_aql(intent: SearchIntent, *, allowed_repos: list[str], max_results: int) -> str:
    ...
    # Numeric filters are only meaningful when positive. A model that emits 0
    # (or a negative) for a "just list them" request would otherwise produce an
    # invalid clause such as {"$before": "0d"}, which Artifactory rejects (400).
    if intent.not_downloaded_for_days is not None and intent.not_downloaded_for_days > 0:
        criteria.append(
            {"stat.downloaded": {"$before": f"{intent.not_downloaded_for_days}d"}}
        )

    if intent.created_before_days is not None and intent.created_before_days > 0:
        criteria.append({"created": {"$before": f"{intent.created_before_days}d"}})

    if intent.min_size_bytes is not None and intent.min_size_bytes > 0:
        criteria.append({"size": {"$gt": intent.min_size_bytes}})

    if intent.max_size_bytes is not None and intent.max_size_bytes > 0:
