...
Only include a filter field when the user explicitly asks for that constraint.
NEVER set a numeric filter to 0. For a generic request to list or show packages or
artifacts with no other constraint, use search_artifacts with an empty search_intent
(optionally just name_pattern:"*" or a limit). If the user asks about repositories
themselves, use list_repositories instead.
    ...
    for a in data.get("actions", []):
        ...
        actions.append(
            PlannedAction(
                tool=a.get("tool", ""),
                args=a.get("args", {}) or {},
                search_intent=_intent_from_dict(si) if si is not None else None,
...
def make_plan(request: str, settings: Settings = default_settings) -> tuple[Plan, str]:
    ...
    if chat is not None:
        try:
            ...
            from .context_optimizer.apply import apply_context_optimizer
